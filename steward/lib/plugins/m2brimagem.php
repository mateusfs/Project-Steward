...
// funções para redimensionamento
	...
	*/
	 ...
	 * @return Boolean/void
	*/	
	public function redimensiona( $nova_largura = 0, $nova_altura = 0, $tipo = '', $proporcional = false)
	{
		...
		if ( !$this->nova_largura && !$this->nova_altura || 
			($this->largura < $this->nova_largura && $this->altura < $this->nova_altura))
		{
			return false;
		} 
		// só passou altura
		...
		if ($proporcional) {
			if ($this->altura > $this->nova_altura) {
				$larguraTemp = $this->largura / ( $this->altura/$this->nova_altura );
				if ($larguraTemp > $this->nova_largura) {
					$this->nova_altura = $this->altura / ( $this->largura/$this->nova_largura );
				} else {
					$this->nova_largura = $this->largura / ( $this->altura/$this->nova_altura );
				}
			...
			if ($this->largura > $this->nova_largura) {
				$alturaTemp = $this->altura / ( $this->largura/$this->nova_largura );
				if ($alturaTemp > $this->nova_altura) {
					$this->nova_largura = $this->largura / ( $this->altura/$this->nova_altura );
				} else {
					$this->nova_altura = $this->altura / ( $this->largura/$this->nova_largura );
				}
