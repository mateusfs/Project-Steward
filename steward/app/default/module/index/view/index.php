<div class="fluent-menu" style="margin-top: -50px;">
	<ul class="tabs-holder">
		<li class="active"><a>Inicio</a>
		</li>
	</ul>
</div>
<div style="padding: 20px 20px 10px;">
	<fieldset>
		<div style="float:left; margin-left:-50px; padding-right: 5%;">
			<nav class="horizontal-menu">
				<ul>
					<li>
					<a class="dropdown-toggle" href="#">Prioridades</a>
					<ul class="dropdown-menu" data-role="dropdown" style="display: none;">
						<?php foreach ($prioridades as $row) {?>
							<li value="<?= $row->getCodigo() ?>"><a href="index/index/prioridade/<?= $row->getCodigo() ?>"><?= $row->getNome() ?></a></li>
						<?php }?>
					</ul>
				</ul>
			</nav>
		...
		<div style="float: right;">
			<div class="button-set" data-role="button-group">
				<button class="large aberto <?= ($aberto)?"active":""; ?>"  data-link="index/index/aberto/1" type="button">Abertos</button>
				<button class="large realizados <?= ($realizado)?"active":""; ?>" data-link="index/index/realizado/3" type="button">Realizados</button>
			</div>
		</div>
	</fieldset>
</div>
		...
		$.Dialog({
			...
			onShow: function(_dialog){
				var content = '<div style="width: 600px; height: 200px; padding: 10px 20px 20px;">'+
						'<form class="user-input" action="index/realizado" method="post" >'+
...
<?php if($sucesso == 2){?>
	new AlertaSucesso("Muito Bem...", "Tarefa realizada!!");
<?php }?>
<?php if($sucesso == 3){?>
	new AlertaSucesso("Muito Obrigado...", "Entraremos em contato!!");
<?php }?>
<?php if($sucesso == 1){?>
	new AlertaSucesso("Sucesso...", "Geração concluida com sucesso..");
<?php }?>
<?php if($error == 1){?>
